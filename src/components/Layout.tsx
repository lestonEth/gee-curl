...
export default function Layout({ children, activeScreen, onScreenChange }: LayoutProps) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      {/* Sidebar */}
      <aside className="w-64 flex flex-col border-r border-surface-container-high bg-surface sticky top-0 h-full py-8 shrink-0">
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        {/* Header */}
        <header className="h-16 flex items-center justify-between px-10 sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-surface-container-high shrink-0">
          <div className="flex items-center gap-6">
            <h2 className="text-xl font-bold font-serif tracking-tight text-on-surface capitalize">Gee Curly {activeScreen}</h2>
          ...
          <div className="flex items-center gap-6">
            ...
            <button className="bg-primary/10 text-primary px-5 py-2 rounded-lg font-sans font-semibold text-sm hover:bg-primary hover:text-on-primary transition-all">
              Quick Appointment
            </button>
